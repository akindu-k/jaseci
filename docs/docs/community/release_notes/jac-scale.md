...
- **Fix: Redis cache path triggered unnecessary MongoDB writes**: `RedisBackend.get()` did not mark deserialized anchors as clean, so every anchor loaded from L2 cache appeared dirty and was re-written to MongoDB on each request. Read-only walkers (e.g., `GetAllTasks`) no longer trigger any database updates.
