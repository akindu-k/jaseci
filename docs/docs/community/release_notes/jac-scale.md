...
- **Fix: Unnecessary writes on read-only requests + in-place mutation persistence**: Anchors loaded from L2 (Redis) cache were not marked clean, causing redundant MongoDB writes on every read-only request. Fixed and added hash-based dirty checking so in-place mutations (`list.append()`, `dict[k]=v`, `set.add()`, nested objects) are now automatically detected and persisted.
